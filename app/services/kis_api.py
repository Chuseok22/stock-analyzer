...
class KISAPIClient:
  ...
  def get_access_token(self) -> str:
    """
    Get access token from Redis cache or issue new token.
    Uses Redis TTL to avoid frequent token requests.
    """
    # Try to get token from Redis cache first
    cached_token = redis_client.get(KIS_TOKEN_REDIS_KEY)
    if cached_token:
      logger.debug("Using cached KIS access token from Redis")
      return cached_token

    # If no cached token, issue new one
    logger.info("Issuing new KIS access token")
    return self._issue_new_token()

  def _issue_new_token(self) -> str:
    """Issue new access token from KIS API and cache it."""
    ...
    try:
      ...
      # Cache token in Redis with TTL (expire 5 minutes before actual expiry for safety)
      cache_ttl = expires_in - 300  # 5 minutes buffer
      if cache_ttl > 0:
        redis_client.set(KIS_TOKEN_REDIS_KEY, access_token, ttl=cache_ttl)
        logger.info(f"KIS access token cached in Redis with TTL: {cache_ttl} seconds")
  ...
  def refresh_token_daily(self) -> bool:
    """
    Force refresh token (called by daily scheduler).
    This ensures we get a fresh token daily as recommended by KIS.
    """
    try:
      # Delete old token from cache
      redis_client.delete(KIS_TOKEN_REDIS_KEY)
      logger.info("Deleted old KIS token from cache")

      # Issue new token
      new_token = self._issue_new_token()

      logger.info("Daily KIS token refresh completed successfully")
      return True

    except Exception as e:
      logger.error(f"Daily KIS token refresh failed: {e}")
      return False
