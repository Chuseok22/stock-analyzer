...
class SchedulingService:
  ...
  def __init__(self):
    ...
    self.kis_api = KISAPIClient()  # For token refresh
  ...
  def setup_schedules(self):
    ...
    try:
      ...
      # KIS token refresh (매일 자정 00:00) - KIS API 권장사항
      self.scheduler.add_job(
          func=self.kis_token_refresh_task,
          trigger=CronTrigger(
              hour=0,  # 자정 00:00
              minute=0
          ),
  ...
  def kis_token_refresh_task(self):
    """Daily task: refresh KIS API access token at midnight (00:00)."""
    logger.info("Starting KIS token refresh task")

    try:
      # Force refresh KIS access token (comply with daily refresh recommendation)
      success = self.kis_api.refresh_token_daily()

      if success:
        logger.info("KIS access token refreshed successfully")

        # Optional: Send admin notification on success
        if settings.send_admin_notifications:
          message = "🔑 KIS API 토큰이 자정에 성공적으로 갱신되었습니다."
          self.notification_service._send_simple_slack_message(message)
      else:
        logger.error("Failed to refresh KIS access token")

        # Send admin notification on failure
        if settings.send_admin_notifications:
          message = "⚠️ KIS API 토큰 갱신에 실패했습니다. 확인이 필요합니다."
          self.notification_service._send_simple_slack_message(message)

    except Exception as e:
      logger.error(f"KIS token refresh task failed: {e}")

      # Send admin notification on exception
      if settings.send_admin_notifications:
        message = f"🚨 KIS API 토큰 갱신 중 오류 발생: {str(e)}"
        self.notification_service._send_simple_slack_message(message)
  ...
  def run_manual_task(self, task_name: str) -> bool:
    ...
    try:
      ...
      elif task_name == "kis_token_refresh":
        self.kis_token_refresh_task()
      else:
